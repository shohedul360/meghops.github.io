...
const fixedPoints = [
    { name: "Toronto", lat: 43.6532, long: -79.3832, size: 0.04, color: 0x6A1B9A },
    { name: "Vancouver", lat: 49.2827, long: -123.1207, size: 0.04, color: 0x6A1B9A },
    { name: "Fort Lauderdale", lat: 26.1224, long: -80.1373, size: 0.04, color: 0x6A1B9A },    
	{ name: "Tbilisi", lat: 41.7151, long: 44.8271, size: 0.04, color: 0x6A1B9A },
    { name: "Ankara", lat: 39.9334, long: 32.8597, size: 0.04, color: 0x6A1B9A },
    { name: "Borjomi", lat: 41.8371, long: 43.3874, size: 0.04, color: 0x6A1B9A },
    { name: "Buenos Aires", lat: -34.6118, long: -58.4173, size: 0.04, color: 0x6A1B9A },
    { name: "Vilnius", lat: 54.6872, long: 25.2797, size: 0.04, color: 0x6A1B9A }
];
...
function addFixedMarkers() {
  ...
  const markerMaterial = new THREE.MeshBasicMaterial({ color: 0x6A1B9A });
  ...
  fixedPoints.forEach((point) => {
    ...
    const name = point.userData ? point.userData.name : '';

    const marker = new THREE.Mesh(markerGeometry, markerMaterial);
    marker.position.set(x, y, z).normalize();
    
    marker.userData.isFixedPoint = true;
